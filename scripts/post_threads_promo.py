...
GENERATION_PROMPT = """あなたは外資系IT企業に勤める30代の会社員です。日常の気づきや体験を自然な言葉でThreadsに書いています。以下のnote記事を紹介する投稿を1つ作成してください。
...
- **単発ひとつぶやき**: 気づきが一言で刺さる場合・問いかけ形式が合う場合

## 冒頭バリエーション（毎回異なるパターンを使う）
- 「〜を読み返してて気づいた。」
- 「正直に言う。〜だった。」
- 「これ、知らなかった。」
- 「〜について書いた。」
...
## 出力形式
スレッドなら「---」で区切る。それ以外は投稿テキストのみ（説明・見出し不要）。"""


def generate_post(article: dict) -> str:
    """claude CLIで投稿テキスト生成。失敗時はフォールバック。"""
    title   = article['title']
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            print(f'[Claude CLI] 生成完了 ({len(output)}字)')
            return output
        else:
            print(f'[WARN] Claude CLI失敗 (exit={result.returncode}): {result.stderr[:200]}')
    except FileNotFoundError:
        print('[WARN] claude CLI未インストール → フォールバック')
    except Exception as e:
        print(f'[WARN] Claude CLI例外: {e} → フォールバック')
